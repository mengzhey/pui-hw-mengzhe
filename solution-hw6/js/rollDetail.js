...
function updatePrice(price){ 
    ...
    function displayFinalPrice(pack){
        ...
        console.log(priceUpdate.finalPack);
        finalPrice = priceUpdate.finalPack * Number(priceUpdate.type);
        finalPriceElement.innerText = finalPrice.toFixed(2);
        };
...
function retrieveFromLocalStorage() {
    ...
    let cartRetrieved = new Array();
    cartRetrieved = JSON.parse(cartString);
    cartFinal = cart.concat(cartRetrieved);
...
function saveToLocalStorage() {
    const cartString = JSON.stringify(cartFinal);
